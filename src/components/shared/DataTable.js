...
const IndeterminateCheckbox = forwardRef((props, ref) => {
  ...
  return (
    <Checkbox className="mb-0" ref={resolvedRef} onChange={(_, e) => handleChange(e)} {...rest} />
  );
});
...
const DataTable = (props) => {
  const {
    ...
    skeletonAvatarProps,
    pagingData,
    onClick, // New prop to handle row click
  } = props;
  ...
  // Add the onClick handler to each row in the TBody
  return (
        ...
        {loading && data.length === 0 ? (
          ...
          <TBody {...getTableBodyProps()}>
            {page.map((row, i) => {
              ...
              // Add onClick prop to each row and call onClick with row data
...
DataTable.propTypes = {
  ...
  onClick: PropTypes.func, // Prop type for onClick function (handlerOpen)
};
